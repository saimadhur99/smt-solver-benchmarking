...
def benchmark_hanoi(num_disks):
    ...
    return result, elapsed_time, stats_str

# Running the benchmark
max_disks = 6
results = []
for num_disks in range(1, max_disks + 1):
    result, time_taken, stats_str = benchmark_hanoi(num_disks)
    results.append((num_disks, result, time_taken, stats_str))

for num_disks, result, time_taken, stats_str in results:
